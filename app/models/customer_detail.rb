class CustomerDetail < ApplicationRecord
  ...
  def update(params)
    Customer.transaction do
      Customer.find(customer_id).update(params.permit(:first_name, :last_name, :username, :email))
      Address.find(billing_address_id).update(address_attributes(params, "billing"))
      Address.find(shipping_address_id).update(address_attributes(params, "shipping"))
    end
  end
